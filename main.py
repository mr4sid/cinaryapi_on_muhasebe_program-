...
from pencereler import (YoneticiAyarlariPenceresi, SirketBilgileriPenceresi, KullaniciYonetimiPenceresi,
                      ...
                      SiparisPenceresi, FaturaGuncellemePenceresi) 
...
class App(ThemedTk):
    ...
    def _check_critical_stock(self):
        ...
        logging.info("Kritik stok ve finansal bildirimler güncellendi.") # Bilgi mesajı logla
    ...
    def _on_tab_change(self, event):
        ...
        # DÜZELTME BAŞLANGICI: _last_selected_tab_widget'ın varlığını ve geçerliliğini kontrol et
        if hasattr(self, '_last_selected_tab_widget') and self._last_selected_tab_widget and self._last_selected_tab_widget.winfo_exists():
            if isinstance(self._last_selected_tab_widget, FaturaOlusturmaSayfasi) or \
               isinstance(self._last_selected_tab_widget, SiparisOlusturmaSayfasi):
                # Sadece yeni bir fatura/sipariş oluşturuluyorsa taslağı kaydet
                # Düzenleme modunda olanları kaydetmemeliyiz.
                if self._last_selected_tab_widget.duzenleme_id is None:
                    self._last_selected_tab_widget._save_current_form_data_to_temp()
                    self.set_status(f"Sekme değiştirildi, {self._last_selected_tab_text.strip()} taslak olarak kaydedildi.")
        # DÜZELTME BİTİŞİ
        ...
        if selected_tab_text == "🏠 33 Ana Sayfa":
        ...
        elif selected_tab_text == "📊 Raporlar": # Raporlama Merkezi Sekmesi
            ...
            self.finansal_islemler_sayfasi.tarih_entry.insert(0, datetime.now().strftime('%Y-%m-%d'))
    ...
    def alis_faturasi_goster(self, initial_tedarikci_id=None, initial_urunler=None): 
        ...
        self._show_or_create_fatura_tab('ALIŞ', initial_tedarikci_id=initial_tedarikci_id, initial_urunler=initial_urunler) 
    ...
    def _show_or_create_fatura_tab(self, fatura_tipi, initial_tedarikci_id=None, initial_urunler=None, initial_data=None, yenile_callback=None): # <-- DÜZELTME: yenile_callback eklendi
        """Ortak metot: Fatura oluşturma sekmesini yönetir.
           Mevcut bir taslak sekmesi varsa ona odaklanır, yoksa yeni bir tane oluşturur.
           Verilerin kaybolmasını engellemek için taslakları otomatik yükler/kaydeder."""

        tab_title_prefix = "Yeni Satış Faturası" if fatura_tipi == 'SATIŞ' else "Yeni Alış Faturası"

        # Açık bir fatura oluşturma sekmesi var mı kontrol et
        for tab_id in self.notebook.tabs():
            tab_text = self.notebook.tab(tab_id, "text")
            if tab_text.startswith(tab_title_prefix) and "Güncelle:" not in tab_text:
                existing_tab_widget = self.notebook.nametowidget(tab_id)
                if isinstance(existing_tab_widget, FaturaOlusturmaSayfasi): # Doğru sınıf adı kontrolü
                    # Mevcut sekme bulundu, ona odaklan.
                    # _load_initial_data metodu otomatik olarak taslak verisi varsa yükleyecektir.
                    self.notebook.select(tab_id)
                    # Eğer initial_data ile geldiysek, var olan sekmedeki taslağı güncelleyebiliriz.
                    if initial_data:
                        existing_tab_widget._load_temp_form_data(forced_temp_data=initial_data)
                    self.set_status(f"Mevcut '{tab_text.strip()}' sekmesine odaklanıldı. Taslak veriler yüklendi.")
                    return # İşlem tamamlandı, yeni sekme oluşturmaya gerek yok.

        fatura_frame = FaturaOlusturmaSayfasi( # Doğru sınıf adı
            self.notebook, # Parent, Notebook'un kendisi
            self.db,
            self, # app_ref
            fatura_tipi,
            duzenleme_id=None, # Yeni bir fatura olduğu için None
            yenile_callback=yenile_callback, # <-- DÜZELTME: yenile_callback buraya iletildi
            initial_cari_id=initial_tedarikci_id, # Bu argüman fatura tipine göre cari ID'sini ayarlar
            initial_urunler=initial_urunler, # Eğer initial_urunler varsa gönderir
            initial_data=initial_data # <-- DÜZELTME: initial_data parametresi eklendi
        )
        tab_title = f"{tab_title_prefix}"
        self.notebook.add(fatura_frame, text=tab_title)
        self.notebook.select(fatura_frame)
        self.set_status(f"Yeni {fatura_tipi.lower()} faturası oluşturma ekranı açıldı.")
