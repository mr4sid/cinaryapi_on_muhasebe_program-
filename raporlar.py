...
class CariYaslandirmaRaporuPenceresi(tk.Toplevel):
    ...
    def _draw_pdf_section(self, canvas, y_start, page_width, section_title, data_dict, styles, styleN, styleH, styleRight, format_currency_func):
        ...
        for period, items in data_dict.items():
            ...
            section_table = Table(section_table_data, colWidths=[8*cm, 3*cm, 3*cm])
            section_table.setStyle(TableStyle([
                ...
                ('FONTNAME', (0,1), (-1,1), 'DejaVuSans-Bold' if 'DejaVuSans-Bold' in pdfmetrics.getRegisteredFontNames() else 'Helvetica-Bold'),
                ('FONTSIZE', (0,1), (-1,1), 8),
                ('BACKGROUND', (0,1), (-1,1), colors.HexColor("#4F81BD")), # Sütun başlıkları arka plan
                ('TEXTCOLOR', (0,1), (-1,1), colors.whitesmoke),
                ('GRID', (0,0), (-1,-1), 0.5, colors.grey),
                ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
                ('FONTNAME', (0,2), (-1,-1), 'DejaVuSans' if 'DejaVuSans' in pdfmetrics.getRegisteredFontNames() else 'Helvetica'), # Veri satırları font
                ('ALIGN', (1,2), (1,-1), 'RIGHT'), # Tutar sağa hizalı
                ('ALIGN', (2,2), (2,-1), 'RIGHT'), # Vadesi Geçen Gün sağa hizalı
            ]))

            # Tabloyu çiz ve y_pos'u güncelle
            ...
            if current_y_for_section - table_height < 50: # Sayfa sonuna çok yakınsa
                ...
                canvas.setFont('DejaVuSans-Bold' if 'DejaVuSans-Bold' in pdfmetrics.getRegisteredFontNames() else 'Helvetica-Bold', 14)
