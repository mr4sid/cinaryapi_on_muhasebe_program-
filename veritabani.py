...
class OnMuhasebe:
    def __init__(self, db_name='on_muhasebe.db'):
        self.app = None
        self.data_dir = data_dir # <-- Burayı güncelleyin
        self.db_name = os.path.join(self.data_dir, db_name)
        print(f"DEBUG: Veritabanı yolu: {self.db_name}")
    ...
    def manuel_stok_hareketi_sil(self, hareket_id):
        ...
        try:
            ...
            stok_fark = 0.0
            if 'Giriş' in islem_tipi or 'Fazlası' in islem_tipi:
                stok_fark = -miktar # Giriş siliniyorsa stoktan düş
            elif 'Çıkış' in islem_tipi or 'Eksiği' in islem_tipi or 'Zayiat' in islem_tipi:
                stok_fark = miktar # Çıkış siliniyorsa stoka ekle
            
            if stok_fark != 0:
                self.c.execute("UPDATE tbl_stoklar SET stok_miktari = stok_miktari + ? WHERE id = ?", (stok_fark, urun_id))
    ...
    def _ensure_perakende_musteri(self):
        # self.connect() # BU SATIRI KESİNLİKLE SİLİN!
        self.c.execute("SELECT id FROM musteriler WHERE kod = ?", (self.PERAKENDE_MUSTERI_KODU,)) # 'musteri_kodu' yerine 'kod'
        ...
        if musteri:
            ...
            try:
                self.c.execute("""
                    INSERT INTO musteriler (ad, kod, notlar, olusturma_tarihi_saat, bakiye)
                    VALUES (?, ?, ?, ?, ?)
                """, ("Perakende Müşteri", self.PERAKENDE_MUSTERI_KODU, "Sistem tarafından otomatik oluşturulan perakende müşteri", datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 0.0))
                self.conn.commit()
                self.perakende_musteri_id = self.c.lastrowid
            except sqlite3.IntegrityError:
                self.conn.rollback() # Hata durumunda rollback
                self.c.execute("SELECT id FROM musteriler WHERE kod = ?", (self.PERAKENDE_MUSTERI_KODU,)) # 'musteri_kodu' yerine 'kod'
                self.perakende_musteri_id = self.c.fetchone()['id']
            except Exception as e:
                print(f"Hata oluştu: {e}")
                self.conn.rollback()

    def _add_column_if_not_exists(self, table_name, column_name, column_type, is_unique=False): # <-- is_unique=False parametresi buraya eklendi
        """
        Belirtilen tabloya belirtilen sütunu ekler, eğer yoksa.
        UNIQUE kısıtlaması için ayrı bir INDEX oluşturur.
        """
        try:
            ...
            if column_name not in columns:
                ...
                self.conn.commit()
                print(f"DEBUG: Sütun eklendi: Tablo={table_name}, Sütun={column_name}")
            ...
            if is_unique:
                try:
                    ...
                    self.conn.commit()
                    print(f"DEBUG: UNIQUE INDEX oluşturuldu: {index_name}")
                except sqlite3.OperationalError as e:
                    # <<<< Hata mesajını daha açıklayıcı hale getirin >>>>
                    print(f"UYARI: UNIQUE INDEX oluşturulurken hata oluştu: {e}. "
                          f"Bu genellikle '{table_name}.{column_name}' sütununda zaten yinelenen veri olduğu anlamına gelir. "
                          f"Verileri manuel olarak kontrol etmeniz gerekebilir.")
                except Exception as e:
                    print(f"UYARI: UNIQUE INDEX oluşturulurken beklenmeyen bir hata oluştu: {e}")
            else:
                print(f"DEBUG: Sütun zaten var: Tablo={table_name}, Sütun={column_name}")
        except sqlite3.OperationalError as e:
            print(f"HATA: Sütun eklenirken veritabanı hatası: {e}")
            # Bu hata, tablonun kilitli olması vb. durumlarda da oluşabilir.
        except Exception as e:
            print(f"HATA: Sütun ekleme kontrolü sırasında beklenmeyen hata: {e}")
    ...
    def create_tables(self):
        ...
        # Müşteriler Tablosu - DÜZELTİLDİ: Son sütundan sonraki fazla virgül kaldırıldı
        self.c.execute(
        ...
        # Tedarikçiler Tablosu - DÜZELTİLDİ: Son sütundan sonraki fazla virgül kaldırıldı
        ...
        self._add_column_if_not_exists('musteriler', 'bakiye', 'REAL DEFAULT 0.0') # Bu satırın musteriler tablosu altına taşınması daha uygun olabilir ama şimdilik burada kalabilir.
        self._add_column_if_not_exists('tedarikciler', 'bakiye', 'REAL DEFAULT 0.0') # Tedarikçiler için de bakiye sütunu ekle
        ...
        self._add_column_if_not_exists('faturalar', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('faturalar', 'orijinal_fatura_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('fatura_kalemleri', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('gelir_gider', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('cari_hareketler', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self.c.execute('''CREATE TABLE IF NOT EXISTS siparisler
                            ...
                            durum TEXT CHECK(durum IN ('BEKLEMEDE', 'TAMAMLANDI', 'İPTAL EDİLDİ')) NOT NULL)''')
        ...
        self._add_column_if_not_exists('siparisler', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('siparis_kalemleri', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('teklifler', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('urun_kategorileri', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('urun_markalari', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('teklif_kalemleri', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('gelir_siniflandirmalari', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self._add_column_if_not_exists('gider_siniflandirmalari', 'olusturan_kullanici_id', 'INTEGER')
        ...
        self.c.execute('''CREATE TABLE IF NOT EXISTS iade_faturalari (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                fatura_no TEXT UNIQUE NOT NULL,
                iade_tarihi TEXT NOT NULL,
                cari_tip TEXT CHECK(cari_tip IN ('MUSTERI', 'TEDARIKCI')) NOT NULL,
                cari_id INTEGER NOT NULL,
                orijinal_fatura_id INTEGER,
                iade_tipi TEXT CHECK(iade_tipi IN ('SATIŞ_İADE', 'ALIŞ_İADE')) NOT NULL,
                toplam_kdv_haric REAL NOT NULL,
                toplam_kdv_dahil REAL NOT NULL,
                aciklama TEXT,
                olusturma_tarihi_saat TEXT,
                ...
                son_guncelleme_tarihi_saat TEXT,
                son_guncelleyen_kullanici_id INTEGER,
                FOREIGN KEY (orijinal_fatura_id) REFERENCES faturalar(id)
            )''')

        # İade Faturaları Tablosuna Denetim Sütunları Ekleme
        self._add_column_if_not_exists('iade_faturalari', 'olusturma_tarihi_saat', 'TEXT')
        self._add_column_if_not_exists('iade_faturalari', 'olusturan_kullanici_id', 'INTEGER')
        self._add_column_if_not_exists('iade_faturalari', 'son_guncelleme_tarihi_saat', 'TEXT')
        self._add_column_if_not_exists('iade_faturalari', 'son_guncelleyen_kullanici_id', 'INTEGER')

        # İade Faturaları için indeksler
        self.c.execute("CREATE INDEX IF NOT EXISTS idx_iade_faturalari_fatura_no ON iade_faturalari (fatura_no);")
        self.c.execute("CREATE INDEX IF NOT EXISTS idx_iade_faturalari_tarih_tip_cari ON iade_faturalari (iade_tarihi, iade_tipi, cari_id);")


        self.c.execute('''CREATE TABLE IF NOT EXISTS iade_fatura_kalemleri (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                iade_fatura_id INTEGER NOT NULL,
                urun_id INTEGER NOT NULL,
                miktar REAL NOT NULL,
                birim_fiyat REAL NOT NULL,
                kdv_orani REAL NOT NULL,
                ...
                kalem_toplam_kdv_haric REAL NOT NULL,
                kalem_toplam_kdv_dahil REAL NOT NULL,
                FOREIGN KEY (iade_fatura_id) REFERENCES iade_faturalari(id) ON DELETE CASCADE,
                FOREIGN KEY (urun_id) REFERENCES tbl_stoklar(id)
            )''')

        # İade Faturası Kalemleri Tablosuna Denetim Sütunları Ekleme
        self._add_column_if_not_exists('iade_fatura_kalemleri', 'olusturma_tarihi_saat', 'TEXT')
        self._add_column_if_not_exists('iade_fatura_kalemleri', 'olusturan_kullanici_id', 'INTEGER')
        self._add_column_if_not_exists('iade_fatura_kalemleri', 'son_guncelleme_tarihi_saat', 'TEXT')
        self._add_column_if_not_exists('iade_fatura_kalemleri', 'son_guncelleyen_kullanici_id', 'INTEGER')

        # İade Faturası Kalemleri için indeksler
        self.c.execute("CREATE INDEX IF NOT EXISTS idx_iade_fatura_kalemleri_fatura_urun ON iade_fatura_kalemleri (iade_fatura_id, urun_id);")
        self.c.execute("CREATE INDEX IF NOT EXISTS idx_iade_fatura_kalemleri_urun_id ON iade_fatura_kalemleri (urun_id);")
        ...
        try:
            ...
            if self.c.fetchone()[0] == 0:
                self.c.execute("INSERT INTO kullanicilar (kullanici_adi, sifre, yetki, olusturma_tarihi_saat) VALUES (?,?,?,?)",
                               ("admin", self._hash_sifre("admin123"), "admin", self.get_current_datetime_str()))
                self.conn.commit()
    ...
    def musteri_ekle(self, kod, ad, telefon, adres, vergi_dairesi, vergi_no):
        ...
        try:
            ...
            current_time = self.get_current_datetime_str()
            olusturan_id = self.app.current_user[0] if self.app and self.app.current_user else None
            self.c.execute("INSERT INTO musteriler (musteri_kodu,ad,telefon,adres,vergi_dairesi,vergi_no,olusturma_tarihi_saat,olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?)",
    ...
    def siparis_ekle(self, siparis_no, siparis_tipi, cari_id, toplam_tutar, durum, kalemler, siparis_notlari=None, teslimat_tarihi=None, genel_iskonto_tipi='YOK', genel_iskonto_degeri=0.0):
        """
        Yeni bir sipariş oluşturur ve ilişkili sipariş kalemi kayıtlarını ekler.
        Dönüş: (bool, siparis_id_veya_hata_mesajı)
        """
        if not (siparis_no and siparis_tipi and cari_id and durum and kalemler):
            return False, "Sipariş No, Sipariş Tipi, Cari ID, Durum ve Kalemler zorunludur."
        ...
        try:
            ...
            if self.c.fetchone():
                self.conn.rollback()
                return False, f"'{siparis_no}' sipariş numarası zaten mevcut. Lütfen başka bir numara girin."

            # `cari_tip` kısıtlamasını karşılamak için siparis_tipi'ne göre dönüştür
            if siparis_tipi == 'SATIŞ_SIPARIS':
                cari_tip_for_db = 'MUSTERI'
            elif siparis_tipi == 'ALIŞ_SIPARIS':
                cari_tip_for_db = 'TEDARIKCI'
            else:
                self.conn.rollback()
                return False, "Geçersiz sipariş tipi. Cari tip 'MUSTERI' veya 'TEDARIKCI' olmalıdır."

            # Toplam tutar hesaplaması
            calculated_total_kdv_dahil = 0.0
            for item in kalemler:
                # item formatı: (urun_id, miktar, birim_fiyat_kdv_haric, kdv_orani, kdv_tutari, kalem_tkh, kalem_tkd,
                # alis_fiyati_siparis_aninda, satis_fiyati_siparis_aninda, iskonto_yuzde_1, iskonto_yuzde_2)
                    calculated_total_kdv_dahil += item[6] # kalem_toplam_kdv_dahil (7. indeks)

            # Genel iskonto hesaplaması
            uygulanan_genel_iskonto_tutari = 0.0
            if genel_iskonto_tipi == 'YUZDE' and genel_iskonto_degeri > 0:
                # Buradaki toplam_tutar_kdv_haric parametresine göre iskontoyu hesapla
                # kalemler listesinden kdv hariç toplamı alalım
                toplam_kdv_haric_kalemler = sum(k[5] for k in kalemler) # kalem_toplam_kdv_haric (6. indeks)
            ...
            final_total_tutar = calculated_total_kdv_dahil - uygulanan_genel_iskonto_tutari
            if final_total_tutar < 0: final_total_tutar = 0.0

            current_time = self.get_current_datetime_str()
            olusturan_id = self.app.current_user[0] if self.app and self.app.current_user else None
            tarih_str = datetime.now().strftime('%Y-%m-%d') # Sipariş tarihi
            ...
            siparis_id = self.c.lastrowid # Yeni oluşturulan siparişin ID'sini al

            # Sipariş kalemlerini tek tek ekle
            for item in kalemler:
                urun_id = item[0]
                miktar = self.safe_float(item[1]) # miktar
                birim_fiyat = self.safe_float(item[2]) # birim_fiyat_kdv_haric
                kdv_orani = self.safe_float(item[3]) # kdv_orani
                kdv_tutari = self.safe_float(item[4]) # kdv_tutari
                kalem_toplam_kdv_haric = self.safe_float(item[5]) # kalem_toplam_kdv_haric
                kalem_toplam_kdv_dahil = self.safe_float(item[6]) # kalem_toplam_kdv_dahil
                alis_fiyati_siparis_aninda = self.safe_float(item[7]) # alis_fiyati_siparis_aninda
                satis_fiyati_siparis_aninda = self.safe_float(item[8]) # satis_fiyati_siparis_aninda
                iskonto_yuzde_1 = self.safe_float(item[9]) # iskonto_yuzde_1
                iskonto_yuzde_2 = self.safe_float(item[10]) # iskonto_yuzde_2

                current_time_item = self.get_current_datetime_str()
                olusturan_id_item = self.app.current_user[0] if self.app and self.app.current_user else None
                ...
                self.c.execute("INSERT INTO siparis_kalemleri (siparis_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_siparis_aninda, satis_fiyati_siparis_aninda, iskonto_yuzde_1, iskonto_yuzde_2, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                 (siparis_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_siparis_aninda, satis_fiyati_siparis_aninda, iskonto_yuzde_1, iskonto_yuzde_2, current_time_item, olusturan_id_item))
            ...
            self.conn.rollback()
            if "UNIQUE constraint failed: siparisler.siparis_no" in str(e):
                return False, f"'{siparis_no}' sipariş numarası zaten mevcut."
            else:
                return False, f"Veritabanı bütünlüğü hatası: {e}"
        except Exception as e:
            self.conn.rollback()
            return False, f"Sipariş oluşturulurken beklenmeyen bir hata oluştu: {e}\n{traceback.format_exc()}"
    ...
    def siparis_guncelle(self, siparis_id, yeni_siparis_no, yeni_siparis_tipi, yeni_cari_id, yeni_toplam_tutar, yeni_durum, yeni_kalemler, yeni_siparis_notlari=None, yeni_teslimat_tarihi=None, genel_iskonto_tipi='YOK', genel_iskonto_degeri=0.0):
        """
        Mevcut bir siparişi ve ilişkili sipariş kalemlerini günceller.
        Dönüş: (bool, mesaj)
        """
        if not (yeni_siparis_no and yeni_siparis_tipi and yeni_cari_id and yeni_durum and yeni_kalemler):
            return False, "Sipariş No, Sipariş Tipi, Cari ID, Durum ve Kalemler zorunludur."

        try:
            self.conn.execute("BEGIN TRANSACTION")

            # Sipariş numarası değiştiyse ve benzersiz değilse kontrol et
            self.c.execute("SELECT siparis_no FROM siparisler WHERE id=?", (siparis_id,))
            current_siparis_no = self.c.fetchone()[0]
            if yeni_siparis_no != current_siparis_no:
                self.c.execute("SELECT id FROM siparisler WHERE siparis_no = ? AND id != ?", (yeni_siparis_no, siparis_id))
                if self.c.fetchone():
                    self.conn.rollback()
                    return False, f"'{yeni_siparis_no}' sipariş numarası zaten başka bir sipariş için mevcut."

            # `cari_tip` kısıtlamasını karşılamak için yeni_siparis_tipi'ne göre dönüştür
            if yeni_siparis_tipi == 'SATIŞ_SIPARIS':
                cari_tip_for_db = 'MUSTERI'
            elif yeni_siparis_tipi == 'ALIŞ_SIPARIS':
                cari_tip_for_db = 'TEDARIKCI'
            else:
                self.conn.rollback()
                return False, "Geçersiz sipariş tipi. Cari tip 'MUSTERI' veya 'TEDARIKCI' olmalıdır."
            ...
            uygulanan_genel_iskonto_tutari = 0.0
            if genel_iskonto_tipi == 'YUZDE' and genel_iskonto_degeri > 0:
                toplam_kdv_haric_kalemler = sum(k[5] for k in yeni_kalemler) # kalem_toplam_kdv_haric (6. indeks)
                uygulanan_genel_iskonto_tutari = toplam_kdv_haric_kalemler * (genel_iskonto_degeri / 100)
            elif genel_iskonto_tipi == 'TUTAR' and genel_iskonto_degeri > 0:
                uygulanan_genel_iskonto_tutari = genel_iskonto_degeri
            ...
            current_time = self.get_current_datetime_str()
            guncelleyen_id = self.app.current_user[0] if self.app and self.app.current_user else None
            tarih_str = datetime.now().strftime('%Y-%m-%d') # Güncelleme tarihi

            # Ana sipariş kaydını güncelle
            self.c.execute("""
                UPDATE siparisler SET 
                    siparis_no=?, tarih=?, cari_tip=?, cari_id=?, toplam_tutar=?, durum=?, 
                    siparis_notlari=?, teslimat_tarihi=?, genel_iskonto_tipi=?, genel_iskonto_degeri=?,
                    son_guncelleme_tarihi_saat=?, son_guncelleyen_kullanici_id=?
            WHERE id=?
            """, (yeni_siparis_no, tarih_str, cari_tip_for_db, yeni_cari_id, final_total_tutar, yeni_durum, 
                  yeni_siparis_notlari, yeni_teslimat_tarihi, genel_iskonto_tipi, genel_iskonto_degeri,
                  current_time, guncelleyen_id, siparis_id))

            # Yeni sipariş kalemlerini ekle
            for item in yeni_kalemler:
                urun_id = item[0]
                miktar = self.safe_float(item[1])
                birim_fiyat = self.safe_float(item[2])
                kdv_orani = self.safe_float(item[3])
                kdv_tutari = self.safe_float(item[4])
                kalem_toplam_kdv_haric = self.safe_float(item[5])
                kalem_toplam_kdv_dahil = self.safe_float(item[6])
                alis_fiyati_siparis_aninda = self.safe_float(item[7])
                satis_fiyati_siparis_aninda = self.safe_float(item[8])
                iskonto_yuzde_1 = self.safe_float(item[9])
                iskonto_yuzde_2 = self.safe_float(item[10])

                current_time_item = self.get_current_datetime_str()
                guncelleyen_id_item = self.app.current_user[0] if self.app and self.app.current_user else None

                self.c.execute("INSERT INTO siparis_kalemleri (siparis_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_siparis_aninda, satis_fiyati_siparis_aninda, iskonto_yuzde_1, iskonto_yuzde_2, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                 (siparis_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_siparis_aninda, satis_fiyati_siparis_aninda, iskonto_yuzde_1, iskonto_yuzde_2, current_time_item, guncelleyen_id_item))
            ...
            self.conn.rollback()
            return False, f"Sipariş güncellenirken beklenmeyen bir hata oluştu: {e}\n{traceback.format_exc()}"
    ...
    def musteri_guncelle(self, id, kod, ad, telefon, adres, vergi_dairesi, vergi_no):
        ...
        try:
            ...
            current_time = self.get_current_datetime_str()
            guncelleyen_id = self.app.current_user[0] if self.app and self.app.current_user else None
            self.c.execute("UPDATE musteriler SET musteri_kodu=?,ad=?,telefon=?,adres=?,vergi_dairesi=?,vergi_no=?,son_guncelleme_tarihi_saat=?,son_guncelleyen_kullanici_id=? WHERE id=?",
    ...
    def musteri_listesi_al(self, arama_terimi="", perakende_haric=False, limit=None, offset=None):
        query = """
            SELECT id, ad, soyad, kod, vergi_dairesi, vergi_no, adres, telefon, email, notlar, bakiye
            FROM musteriler
        """
        params = []
        
        if perakende_haric:
            query += " WHERE kod != ?" # musteri_kodu yerine KOD kullanıyoruz
    ...
    def get_musteri_count(self, arama_terimi=None, perakende_haric=False): # Yeni metot: toplam müşteri sayısını alır
        query = "SELECT COUNT(id) FROM musteriler"
        params = []
        conditions = []
        ...
        if perakende_haric and self.perakende_musteri_id is not None:
            ...
            params.append(self.perakende_musteri_id)
            
        if arama_terimi:
            term = f"%{arama_terimi}%"
            conditions.append("(musteri_kodu LIKE ? OR ad LIKE ? OR telefon LIKE ? OR adres LIKE ?)")
    ...
    def musteri_getir_by_id(self, musteri_id):
        self.c.execute("SELECT id,musteri_kodu,ad,telefon,adres,vergi_dairesi,vergi_no FROM musteriler WHERE id=?",(musteri_id,))
        result = self.c.fetchone()
        # return result # Doğrudan Row objesini döndürmek daha iyidir
        if result:
            print(f"DEBUG: Müşteri adı: {result['ad']}, Kodu: {result['musteri_kodu']}")
        return result
    ...
    def tedarikci_listesi_al(self, arama_terimi=None, limit=None, offset=None): 
        ...
        params = []
        if arama_terimi:
            query += " WHERE tedarikci_kodu LIKE ? OR ad LIKE ? OR telefon LIKE ? OR adres LIKE ?"
            term = f"%{arama_terimi}%"
            params.extend([term, term, term, term])
    ...
    def get_tedarikci_count(self, arama_terimi=None): # toplam tedarikçi sayısını alır
    ...
    def kategori_ekle(self, kategori_adi):
        ...
        try:
            ...
            # Sadece başarı durumunu ve bir mesaj döndür
    ...
    def stok_ekle(self, kod, ad, stok_miktari, alis_haric, satis_haric, kdv_orani, min_stok_seviyesi, alis_kdv_dahil, satis_kdv_dahil, kategori_id=None, marka_id=None, urun_detayi=None, urun_resmi_yolu=None, fiyat_degisiklik_tarihi=None, urun_grubu_id=None, urun_birimi_id=None, ulke_id=None):
        ...
        try:
            s_m=float(str(stok_miktari).replace(',','.'))
            a_f_h=float(str(alis_haric).replace(',','.')) # KDV Hariç Alış Fiyatı
            s_f_h=float(str(satis_haric).replace(',','.')) # KDV Hariç Satış Fiyatı
            k_o=float(str(kdv_orani).replace(',','.'))
            min_s_s=float(str(min_stok_seviyesi).replace(',','.'))
            a_f_d=float(str(alis_kdv_dahil).replace(',','.')) # KDV Dahil Alış Fiyatı
            s_f_d=float(str(satis_kdv_dahil).replace(',','.')) # KDV Dahil Satış Fiyatı
        except ValueError:
        ...
        try:
            ...
            yeni_urun_id = self.c.lastrowid # Yeni ID'yi al
            self.conn.commit()
            
            # <<< HATA BURADAYDI! Artık (bool, id) formatında bir tuple döndürüyoruz. >>>
    ...
    def stok_guncelle(self, id, kod, ad, stok_miktari, alis_haric, satis_haric, kdv_orani, min_stok_seviyesi, alis_kdv_dahil, satis_kdv_dahil, kategori_id=None, marka_id=None, urun_detayi=None, urun_resmi_yolu=None, fiyat_degisiklik_tarihi=None, urun_grubu_id=None, urun_birimi_id=None, ulke_id=None):
        if not (kod and ad):
            return False, "Ürün Kodu ve Adı zorunludur."
        try:
            yeni_stok_miktari_f = float(str(stok_miktari).replace(',', '.'))
            a_f_h = float(str(alis_haric).replace(',', '.'))
            s_f_h = float(str(satis_haric).replace(',', '.'))
            k_o = float(str(kdv_orani).replace(',', '.'))
            min_s_s = float(str(min_stok_seviyesi).replace(',', '.'))
            a_f_d = float(str(alis_kdv_dahil).replace(',', '.'))
            s_f_d = float(str(satis_kdv_dahil).replace(',', '.'))
        except ValueError:
            return False, "Sayısal alanlar doğru formatta olmalıdır."
        ...
        try:
            ...
            # ADIM 2: Ürünü normal şekilde güncelle
            current_time = self.get_current_datetime_str()
            guncelleyen_id = self.app.current_user[0] if self.app and self.app.current_user else None
        
            self.c.execute("""
                UPDATE tbl_stoklar SET 
                    urun_kodu=?, urun_adi=?, stok_miktari=?, alis_fiyati_kdv_haric=?, 
                    satis_fiyati_kdv_haric=?, kdv_orani=?, min_stok_seviyesi=?, 
                    alis_fiyati_kdv_dahil=?, satis_fiyati_kdv_dahil=?, kategori_id=?, 
                    ...
                    son_guncelleme_tarihi_saat=?, son_guncelleyen_kullanici_id=? 
                WHERE id=?
            """, (kod, ad, yeni_stok_miktari_f, a_f_h, s_f_h, k_o, min_s_s, a_f_d, s_f_d, 
            ...
            # ADIM 3: Stok miktarını karşılaştır ve gerekirse stok hareketi ekle
            ...
            if stok_farki != 0:
                ...
                hareket_aciklamasi = ""
                if stok_farki > 0:
                    islem_tipi = "Giriş (Manuel Düzeltme)"
                    hareket_aciklamasi = f"Ürün kartından manuel stok artışı. Fark: +{stok_farki:.2f}"
                else: # stok_farki < 0
                    islem_tipi = "Çıkış (Manuel Düzeltme)"
                    hareket_aciklamasi = f"Ürün kartından manuel stok azalışı. Fark: {stok_farki:.2f}"
                ...
                self.c.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (id, datetime.now().strftime('%Y-%m-%d'), islem_tipi, abs(stok_farki), eski_stok_miktari_f, yeni_stok_miktari_f, hareket_aciklamasi, "MANUEL", None, current_time, guncelleyen_id))
            ...
            self.conn.rollback()
            return False, f"Stok güncelleme sırasında hata: {e}"
    ...
    def stok_hareketi_ekle(self, urun_id, islem_tipi, miktar, tarih, aciklama=""):
        ...
        try:
            ...
            # ### HATA DÜZELTMESİ BURADA ###
            # INSERT komutuna eksik olan "kaynak_id" sütunu eklendi ve
            # VALUES kısmına bu sütun için "None" değeri verildi.
            self.c.execute("""
                INSERT INTO stok_hareketleri (urun_id, tarih, islem_tipi, miktar, onceki_stok, sonraki_stok, aciklama, kaynak, kaynak_id, olusturma_tarihi_saat, olusturan_kullanici_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def geriye_donuk_stok_hareketlerini_olustur(self):
        ...
        try:
            ...
            cursor_thread.execute("DELETE FROM stok_hareketleri WHERE kaynak = 'FATURA'")
            print("Mevcut fatura kaynaklı stok hareketleri temizlendi.")

            # Tüm faturaları ve kalemlerini çek
            ...
            for kalem in fatura_kalemleri:
                fatura_id, fatura_no, tarih, tip, urun_id, miktar = kalem

                onceki_stok = 0.0
                sonraki_stok = 0.0

                islem_tipi_hareket = "Fatura Satış" if tip == 'SATIŞ' else "Fatura Alış"
                aciklama_hareket = f"Geçmiş Fatura No: {fatura_no}"

                # Geçerli kullanıcı ID'sini al (varsa)
                olusturan_id = self.app.current_user[0] if self.app and hasattr(self.app, 'current_user') and self.app.current_user else None

                cursor_thread.execute("""
                    INSERT INTO stok_hareketleri 
                    (urun_id, tarih, islem_tipi, miktar, onceki_stok, sonraki_stok, aciklama, kaynak, kaynak_id, olusturma_tarihi_saat, olusturan_kullanici_id)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (urun_id, tarih, islem_tipi_hareket, miktar, onceki_stok, sonraki_stok, aciklama_hareket, 'FATURA', fatura_id, self.get_current_datetime_str(), olusturan_id))
                hareket_sayisi += 1

            conn_thread.commit()
            return True, f"Geçmişe dönük {hareket_sayisi} adet stok hareketi başarıyla oluşturuldu."
            ...
            # Adım 2: İşlem bitince (hata olsa da olmasa da) geçici bağlantıyı mutlaka kapat.
            if conn_thread:
                conn_thread.close()
    ...
    def stok_sil(self, urun_id):
        try:
            self.conn.execute("BEGIN TRANSACTION")
            # Ürünün fatura, sipariş, teklif kalemlerinde kullanılıp kullanılmadığını kontrol et
            for tablo_adi in ["fatura_kalemleri", "siparis_kalemleri", "teklif_kalemleri"]:
                self.c.execute("SELECT COUNT(*) FROM stok_hareketleri WHERE urun_id=?", (urun_id,))
                if self.c.fetchone()[0] > 0:
                    self.conn.rollback()
                    return False, "Bu ürün 'stok_hareketleri' tablosunda kullanılıyor.\nÖnce ilgili kayıtları düzenlemelisiniz."
    ...
    def stok_getir_for_fatura(self, fatura_tipi='SATIŞ', arama_terimi=None): # <-- DÜZELTME: 'arama_termi' -> 'arama_terimi'
        ...
        print(f"DEBUG_DB: Query: {query}")
        print(f"DEBUG_DB: Params: {params}")

        self.c.execute(query, params)
        results = self.c.fetchall()

        print(f"DEBUG_DB: Sorgu sonuçları (ilk 5 kayıt): {results[:5]}")
        # Her bir sonucun fiyat kolonundaki değerini de kontrol edelim
        for idx, row in enumerate(results):
            if idx < 5: # İlk 5 kaydı detaylı incele
                print(f"DEBUG_DB: Row {idx}: id={row[0]}, kod={row[1]}, ad={row[2]}, fiyat={row[3]}, kdv={row[4]}, stok={row[5]}")
    ...
    def _stok_guncelle_ve_hareket_kaydet(self, kaynak_tipi, kaynak_id, urun_id, miktar, fatura_tipi, referans_no):
        """
        Stok miktarını günceller ve stok hareketleri tablosuna bir kayıt ekler.
        Bu bir yardımcı metottur ve bir transaction içinde çağrılmalıdır.
        """
        self.c.execute("SELECT stok_miktari FROM tbl_stoklar WHERE id=?", (urun_id,))
        stok_oncesi_tuple = self.c.fetchone()
        stok_oncesi = stok_oncesi_tuple[0] if stok_oncesi_tuple else 0.0

        stok_sonrasi = 0.0
        islem_tipi_hareket = ""
        
        if fatura_tipi == 'SATIŞ':
            stok_sonrasi = stok_oncesi - miktar
            ...
            self.c.execute("UPDATE tbl_stoklar SET stok_miktari = ? WHERE id=?", (stok_sonrasi, urun_id))
        elif kaynak_tipi == 'FATURA_SILME' and fatura_tipi == 'SATIŞ': # Fatura silme ters işlem
            stok_sonrasi = stok_oncesi + miktar
            islem_tipi_hareket = "Fatura Satış İptal"
            self.c.execute("UPDATE tbl_stoklar SET stok_miktari = ? WHERE id=?", (stok_sonrasi, urun_id))
        elif kaynak_tipi == 'FATURA_SILME' and fatura_tipi == 'ALIŞ': # Fatura silme ters işlem
            stok_sonrasi = stok_oncesi - miktar
            islem_tipi_hareket = "Fatura Alış İptal"
            self.c.execute("UPDATE tbl_stoklar SET stok_miktari = ? WHERE id=?", (stok_sonrasi, urun_id))

        aciklama_hareket = f"{fatura_tipi} Faturası No: {referans_no}"
        ...
        self.c.execute("""
            ...
            (urun_id, tarih, islem_tipi, miktar, onceki_stok, sonraki_stok, aciklama, kaynak, kaynak_id, olusturma_tarihi_saat, olusturan_kullanici_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (urun_id, tarih_str, islem_tipi_hareket, miktar, stok_oncesi, stok_sonrasi, aciklama_hareket, 'FATURA', kaynak_id, current_time, olusturan_id))

    def fatura_olustur(self, fatura_no, tip, cari_id, kalemler, odeme_turu, kasa_banka_id=None, misafir_adi=None, fatura_notlari=None, vade_tarihi=None, genel_iskonto_tipi='YOK', genel_iskonto_degeri=0.0, original_fatura_id=None, manage_transaction=True): # <-- DÜZELTME: original_fatura_id eklendi
        is_perakende_satis = False
        if tip == 'SATIŞ' and self.perakende_musteri_id is not None and str(cari_id) == str(self.perakende_musteri_id):
            is_perakende_satis = True

        if tip == 'SATIŞ' and is_perakende_satis and odeme_turu == "AÇIK HESAP":
            return False, "Perakende satışlarda 'AÇIK HESAP' ödeme türü kullanılamaz."
        ...
        try:
            ...
            for item in kalemler:
                miktar_f = self.safe_float(item[1])
                birim_fiyat_kdv_haric_f = self.safe_float(item[2])
                kdv_orani_f = self.safe_float(item[3])
                iskonto_yuzde_1_f = self.safe_float(item[5])
                iskonto_yuzde_2_f = self.safe_float(item[6])
                orijinal_birim_fiyat_kdv_dahil = birim_fiyat_kdv_haric_f * (1 + kdv_orani_f / 100)
                fiyat_iskonto_1_sonrasi = orijinal_birim_fiyat_kdv_dahil * (1 - iskonto_yuzde_1_f / 100)
                iskontolu_birim_fiyat_dahil = fiyat_iskonto_1_sonrasi * (1 - iskonto_yuzde_2_f / 100)
                if kdv_orani_f == 0: iskontolu_birim_fiyat_haric = iskontolu_birim_fiyat_dahil
                else: iskontolu_birim_fiyat_haric = iskontolu_birim_fiyat_dahil / (1 + kdv_orani_f / 100)
                toplam_kdv_haric_kalemler += miktar_f * iskontolu_birim_fiyat_haric
                toplam_kdv_dahil_kalemler += miktar_f * iskontolu_birim_fiyat_dahil
            ...
            nihai_toplam_kdv_haric_fatura = toplam_kdv_haric_kalemler - uygulanan_genel_iskonto_tutari
            nihai_toplam_kdv_dahil_fatura = toplam_kdv_dahil_kalemler - uygulanan_genel_iskonto_tutari

            current_time = self.get_current_datetime_str()
            olusturan_id = self.app.current_user[0] if self.app and self.app.current_user else None
            tarih_str = datetime.now().strftime('%Y-%m-%d')

            # DÜZELTME BAŞLANGICI: INSERT sorgusuna original_fatura_id eklendi
            self.c.execute("INSERT INTO faturalar (fatura_no, tarih, tip, cari_id, toplam_kdv_haric, toplam_kdv_dahil, odeme_turu, misafir_adi, kasa_banka_id, olusturma_tarihi_saat, olusturan_kullanici_id, fatura_notlari, vade_tarihi, genel_iskonto_tipi, genel_iskonto_degeri, orijinal_fatura_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                           (fatura_no, tarih_str, tip, cari_id, nihai_toplam_kdv_haric_fatura, nihai_toplam_kdv_dahil_fatura, odeme_turu, misafir_adi, kasa_banka_id, current_time, olusturan_id, fatura_notlari, vade_tarihi, genel_iskonto_tipi, genel_iskonto_degeri, original_fatura_id))
            # DÜZELTME BİTİŞİ
            fatura_id = self.c.lastrowid

            for urun_id, miktar_val, birim_fiyat_kdv_haric_val, kdv_orani_val, alis_fiyati_fatura_aninda_val, iskonto_yuzde_1_val, iskonto_yuzde_2_val, iskonto_tipi_genel_val, iskonto_degeri_genel_val in kalemler:
                miktar_f = self.safe_float(miktar_val)
                birim_fiyat_kdv_haric_f = self.safe_float(birim_fiyat_kdv_haric_val)
                kdv_orani_f = self.safe_float(kdv_orani_val)
                alis_fiyati_fatura_aninda_f = self.safe_float(alis_fiyati_fatura_aninda_val)
                iskonto_yuzde_1_f = self.safe_float(iskonto_yuzde_1_val)
                iskonto_yuzde_2_f = self.safe_float(iskonto_yuzde_2_val)
                
                orijinal_birim_fiyat_kdv_dahil = birim_fiyat_kdv_haric_f * (1 + kdv_orani_f / 100)
                fiyat_iskonto_1_sonrasi_dahil = orijinal_birim_fiyat_kdv_dahil * (1 - iskonto_yuzde_1_f / 100)
                iskontolu_birim_fiyat_dahil = fiyat_iskonto_1_sonrasi_dahil * (1 - iskonto_yuzde_2_f / 100)
                if kdv_orani_f == 0: iskontolu_birim_fiyat_haric = iskontolu_birim_fiyat_dahil
                else: iskontolu_birim_fiyat_haric = iskontolu_birim_fiyat_dahil / (1 + kdv_orani_f / 100)
                kalem_kdv_tutari = (iskontolu_birim_fiyat_dahil - iskontolu_birim_fiyat_haric) * miktar_f
                kalem_toplam_kdv_haric = iskontolu_birim_fiyat_haric * miktar_f
                kalem_toplam_kdv_dahil = iskontolu_birim_fiyat_dahil * miktar_f
                
                self.c.execute("INSERT INTO fatura_kalemleri (fatura_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_fatura_aninda, iskonto_yuzde_1, iskonto_yuzde_2, iskonto_tipi, iskonto_degeri, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               (fatura_id, urun_id, miktar_f, birim_fiyat_kdv_haric_f, kdv_orani_f, kalem_kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_fatura_aninda_f, iskonto_yuzde_1_f, iskonto_yuzde_2_f, iskonto_tipi_genel_val, iskonto_degeri_genel_val, current_time, olusturan_id))
                
                # Önceki dağınık blok yerine tek satırlık temiz bir çağrı
                self._stok_guncelle_ve_hareket_kaydet('FATURA', fatura_id, urun_id, miktar_f, tip, fatura_no)
            
            self._fatura_finansal_etki_olustur(fatura_id, fatura_no, tip, tarih_str, cari_id, nihai_toplam_kdv_dahil_fatura, odeme_turu, kasa_banka_id, is_perakende_satis)

            if manage_transaction: self.conn.commit()
            return True, fatura_id

        except Exception as e:
            if manage_transaction: self.conn.rollback()
            error_details = traceback.format_exc()
            logging.error(f"Fatura oluşturulurken beklenmeyen bir hata oluştu: {e}\nDetaylar: {error_details}")
            return False, f"Fatura oluşturulurken beklenmeyen bir hata oluştu. Detaylar için log dosyasına bakınız."

    def fatura_iade_olustur(self, original_fatura_id, iade_tarihi_str, iade_notlari=None):
        """
        Mevcut bir fatura için iade faturası oluşturur.
        Stokları ve finansal hareketleri tersine çevirir.
        
        Args:
            original_fatura_id (int): İade edilecek orijinal faturanın ID'si.
            iade_tarihi_str (str): İade faturasının tarihi (YYYY-AA-GG).
            iade_notlari (str, optional): İade faturası için ek notlar. Defaults to None.
        ...
        try:
            ...
            # 1. Orijinal Fatura Bilgilerini Çek
            original_fatura = self.fatura_getir_by_id(original_fatura_id)
            if not original_fatura:
                self.conn.rollback()
                return False, "İade edilecek orijinal fatura bulunamadı."
            
            # Orijinal fatura türüne göre iade faturası türünü belirle
            if original_fatura['tip'] == 'SATIŞ':
                iade_fatura_tipi = 'İSATIŞ İADE'
                new_fatura_no_prefix = "IS" # İade Satış
            elif original_fatura['tip'] == 'ALIŞ':
                iade_fatura_tipi = 'ALIŞ İADE'
                new_fatura_no_prefix = "IA" # İade Alış
            else:
                self.conn.rollback()
                return False, f"'{original_fatura['tip']}' tipindeki fatura için iade oluşturulamaz."

            # Aynı orijinal faturaya daha önce iade yapılmış mı kontrol et
            self.c.execute("SELECT id FROM faturalar WHERE orijinal_fatura_id = ?", (original_fatura_id,))
            if self.c.fetchone():
                self.conn.rollback()
                return False, "Bu faturaya zaten bir iade faturası kesilmiş."

            # 2. İade Faturası için Yeni Fatura Numarası Oluştur
            iade_fatura_no = f"{new_fatura_no_prefix}-{original_fatura['fatura_no']}-{datetime.now().strftime('%H%M%S%f')}"
            
            # 3. Yeni İade Faturasını faturalar Tablosuna Ekle
            # Toplam tutarlar, orijinal faturanın toplam tutarlarıyla aynı olacak.
            # Ancak muhasebe etkisi tersine dönecek (gelir - gider, alacak - borç).
            ...
            self.c.execute("""
                INSERT INTO faturalar (
                    fatura_no, tarih, tip, cari_id, toplam_kdv_haric, toplam_kdv_dahil, 
                    odeme_turu, misafir_adi, kasa_banka_id, fatura_notlari, vade_tarihi, 
                    genel_iskonto_tipi, genel_iskonto_degeri, orijinal_fatura_id, 
                    olusturma_tarihi_saat, olusturan_kullanici_id
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                iade_fatura_no, 
                iade_tarihi_str, 
                iade_fatura_tipi, 
                original_fatura['cari_id'], 
                original_fatura['toplam_kdv_haric'], 
                original_fatura['toplam_kdv_dahil'],
                original_fatura['odeme_turu'], # Orijinal ödeme türünü koru
                original_fatura['misafir_adi'],
                original_fatura['kasa_banka_id'],
                f"Orijinal Fatura No: {original_fatura['fatura_no']}. {iade_notlari or ''}".strip(), # Notlara orijinal fatura no ekle
                original_fatura['vade_tarihi'],
                original_fatura['genel_iskonto_tipi'],
                original_fatura['genel_iskonto_degeri'],
                original_fatura_id, # orijinal_fatura_id'yi kaydet
                current_time, 
                olusturan_id
            ))
            new_iade_fatura_id = self.c.lastrowid

            # 4. Orijinal Faturanın Kalemlerini Çek ve İade Faturası için Kaydet
            original_kalemler = self.fatura_detay_al(original_fatura_id)
            for kalem in original_kalemler:
                # İade faturası kalemi için değerleri al (miktar negatif olmayacak, normal pozitif miktar)
                self.c.execute("INSERT INTO fatura_kalemleri (fatura_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_fatura_aninda, kdv_orani_fatura_aninda, iskonto_yuzde_1, iskonto_yuzde_2, iskonto_tipi, iskonto_degeri, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               (new_iade_fatura_id, kalem['urun_id'], kalem['miktar'], kalem['birim_fiyat'], kalem['kdv_orani'], kalem['kdv_tutari'], kalem['kalem_toplam_kdv_haric'], kalem['kalem_toplam_kdv_dahil'], kalem['alis_fiyati_fatura_aninda'], kalem['kdv_orani_fatura_aninda'], kalem['iskonto_yuzde_1'], kalem['iskonto_yuzde_2'], kalem['iskonto_tipi'], kalem['iskonto_degeri'], current_time, olusturan_id))

                # 5. Stok Hareketlerini Tersine Çevir
                # Orijinal SATIŞ ise stok artacak (iade), ALIŞ ise stok azalacak (iade)
                stok_hareket_tipi = ""
                if original_fatura['tip'] == 'SATIŞ':
                    stok_hareket_tipi = "Fatura İade Giriş"
                    self.c.execute("UPDATE tbl_stoklar SET stok_miktari = stok_miktari + ? WHERE id=?", (kalem['miktar'], kalem['urun_id']))
                elif original_fatura['tip'] == 'ALIŞ':
                    stok_hareket_tipi = "Fatura İade Çıkış"
                    self.c.execute("UPDATE tbl_stoklar SET stok_miktari = stok_miktari - ? WHERE id=?", (kalem['miktar'], kalem['urun_id']))
                
                # Yeni stok hareketi kaydı oluştur
                self.c.execute("""
                    INSERT INTO stok_hareketleri 
                    (urun_id, tarih, islem_tipi, miktar, onceki_stok, sonraki_stok, aciklama, kaynak, kaynak_id, olusturma_tarihi_saat, olusturan_kullanici_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (kalem['urun_id'], iade_tarihi_str, stok_hareket_tipi, kalem['miktar'], 0, 0, # Onceki/Sonraki stok dinamik olarak doldurulmalı (basitlik için 0)
                      f"İade Faturası No: {iade_fatura_no}", 'İADE_FATURA', new_iade_fatura_id, current_time, olusturan_id))

            # 6. Finansal Etkileri Tersine Çevir
            # Orijinal faturanın toplam tutarını kullanarak finansal hareketler oluştur.
            # Kasa/Banka hareketleri ve cari hareketler.

            cari_tip_str = 'MUSTERI' if original_fatura['tip'] == 'SATIŞ' else 'TEDARIKCI'
            
            # Eğer orijinal fatura peşin ise, Kasa/Banka ve Gelir/Gider kayıtlarını tersine çevir.
            if original_fatura['odeme_turu'] in self.pesin_odeme_turleri:
                # Gelir/Gider kaydını tersine çevir
                gg_tip_iade = 'GİDER' if original_fatura['tip'] == 'SATIŞ' else 'GELİR' # Satış iadesi gider, alış iadesi gelir
                gg_aciklama_iade = f"İade Faturası: {iade_fatura_no} - Orijinal: {original_fatura['fatura_no']}"
                self.c.execute("INSERT INTO gelir_gider (tarih,tip,tutar,aciklama,kaynak,kaynak_id, kasa_banka_id,olusturma_tarihi_saat,olusturan_kullanici_id, gelir_siniflandirma_id, gider_siniflandirma_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                               (iade_tarihi_str, gg_tip_iade, original_fatura['toplam_kdv_dahil'], gg_aciklama_iade, 'İADE_FATURA', new_iade_fatura_id, original_fatura['kasa_banka_id'], current_time, olusturan_id, None, None))
                
                # Kasa/Banka bakiyesini tersine çevir
                if original_fatura['kasa_banka_id']:
                    is_bakiye_ters_etki = False if original_fatura['tip'] == 'SATIŞ' else True # Satış iadesi bakiyeyi AZALTIR, alış iadesi ARTIRIR
                    self.kasa_banka_bakiye_guncelle(original_fatura['kasa_banka_id'], original_fatura['toplam_kdv_dahil'], artir=is_bakiye_ters_etki)
                
                # Cari hareket kaydını tersine çevir
                ch_islem_tipi_iade = 'BORC' if original_fatura['tip'] == 'SATIŞ' else 'ALACAK' # Satış iadesi müşteriye borç, alış iadesi bize alacak
                ch_aciklama_iade = f"İade Faturası ({iade_fatura_no}) etkisi - Orijinal: {original_fatura['fatura_no']}"
                self.c.execute("INSERT INTO cari_hareketler (tarih, cari_tip, cari_id, islem_tipi, tutar, aciklama, referans_id, referans_tip, kasa_banka_id, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                               (iade_tarihi_str, cari_tip_str, original_fatura['cari_id'], ch_islem_tipi_iade, original_fatura['toplam_kdv_dahil'], ch_aciklama_iade, new_iade_fatura_id, 'İADE_FATURA', original_fatura['kasa_banka_id'], current_time, olusturan_id))

            # Eğer orijinal fatura "AÇIK HESAP" ise, yeni bir cari hareket kaydı oluşturarak borcu/alacağı tersine çevir.
            elif original_fatura['odeme_turu'] == 'AÇIK HESAP':
                ch_islem_tipi_iade_acik_hesap = 'BORC' if original_fatura['tip'] == 'SATIŞ' else 'ALACAK' # Satış iadesi müşteriye borç, alış iadesi bize alacak
                ch_aciklama_iade_acik_hesap = f"İade Faturası ({iade_fatura_no}) etkisi - Orijinal: {original_fatura['fatura_no']}"
                self.c.execute("INSERT INTO cari_hareketler (tarih, cari_tip, cari_id, islem_tipi, tutar, aciklama, referans_id, referans_tip, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?,?)",
                               (iade_tarihi_str, cari_tip_str, original_fatura['cari_id'], ch_islem_tipi_iade_acik_hesap, original_fatura['toplam_kdv_dahil'], ch_aciklama_iade_acik_hesap, new_iade_fatura_id, 'İADE_FATURA', current_time, olusturan_id))

            self.conn.commit() # Tüm işlemler başarılıysa commit et
            return True, f"'{original_fatura['fatura_no']}' numaralı faturaya ait iade faturası ('{iade_fatura_no}') başarıyla oluşturuldu."

        except Exception as e:
            self.conn.rollback() # Hata durumunda tüm işlemi geri al
            error_details = traceback.format_exc()
            logging.error(f"Fatura iade oluşturulurken hata: {e}\nDetaylar: {error_details}")
            return False, f"Fatura iade oluşturulurken beklenmeyen bir hata oluştu: {e}. Detaylar için log dosyasına bakınız."

    def _fatura_finansal_etki_olustur(self, fatura_id, fatura_no, fatura_tipi, tarih, cari_id, tutar, odeme_turu, kasa_banka_id, is_perakende_satis):
        """Bir fatura oluşturulduğunda veya güncellendiğinde finansal etkileri yönetir."""
        olusturan_id = self.app.current_user[0] if self.app and self.app.current_user else None
        current_time = self.get_current_datetime_str()
        
        cari_tip_str = 'MUSTERI' if fatura_tipi == 'SATIŞ' else 'TEDARIKCI'
        
        # Cari adını sadece bir kez, en başta alalım.
        cari_bilgi = self.musteri_getir_by_id(cari_id) if fatura_tipi == 'SATIŞ' else self.tedarikci_getir_by_id(cari_id)
        cari_adi_hareket = cari_bilgi['ad'] if cari_bilgi else ""
        
        # <<< MANTIK BURADA YENİDEN DÜZENLENDİ >>>

        # 1. Adım: Fatura her zaman (ETKİSİZ hariç) bir BORÇ/ALACAK hareketi oluşturur.
        # Bu, faturanın kendisini temsil eder.
        if odeme_turu != 'ETKİSİZ FATURA' and not is_perakende_satis:
            islem_tipi_borc_alacak = 'ALACAK' if fatura_tipi == 'SATIŞ' else 'BORC'
            aciklama_borc_alacak = f"{fatura_tipi} Faturası No: {fatura_no} - Cari: {cari_adi_hareket}"
            self.c.execute("INSERT INTO cari_hareketler (tarih, cari_tip, cari_id, islem_tipi, tutar, aciklama, referans_id, referans_tip, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?,?)",
                           (tarih, cari_tip_str, cari_id, islem_tipi_borc_alacak, tutar, aciklama_borc_alacak, fatura_id, 'FATURA', current_time, olusturan_id))

        # 2. Adım: Eğer ödeme peşin ise, Kasa/Banka ve Gelir/Gider kayıtları oluşturulur.
        # Ayrıca carinin borcunu kapatmak için bir de tahsilat/ödeme hareketi oluşturulur.
        if odeme_turu in self.pesin_odeme_turleri:
            gg_tip = 'GELİR' if fatura_tipi == 'SATIŞ' else 'GİDER'
            gg_aciklama = f"Peşin {fatura_tipi} Faturası: {fatura_no} - Cari: {cari_adi_hareket}"
            
            # Gelir/Gider kaydını oluştur
            self.c.execute("INSERT INTO gelir_gider (tarih,tip,tutar,aciklama,kaynak,kaynak_id, kasa_banka_id,olusturma_tarihi_saat,olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?)",
                           (tarih, gg_tip, tutar, gg_aciklama, 'FATURA', fatura_id, kasa_banka_id, current_time, olusturan_id))
            
            # Kasa/Banka bakiyesini güncelle
            if kasa_banka_id:
                is_bakiye_artir = (gg_tip == 'GELİR')
                self.kasa_banka_bakiye_guncelle(kasa_banka_id, tutar, artir=is_bakiye_artir)
            
            # Cari borcunu kapatan tahsilat/ödeme hareketini ekle (Perakende hariç)
            if not is_perakende_satis:
                islem_tipi_ch_pesin = 'TAHSILAT' if fatura_tipi == 'SATIŞ' else 'ODEME'
                referans_tip_pesin = 'FATURA_SATIS_PESIN' if fatura_tipi == 'SATIŞ' else 'FATURA_ALIS_PESIN'
                aciklama_pesin = f"Peşin Fatura Ödemesi: {fatura_no} ({odeme_turu}) - Cari: {cari_adi_hareket}"
                self.c.execute("INSERT INTO cari_hareketler (tarih,cari_tip,cari_id,islem_tipi,tutar,aciklama,referans_id,referans_tip,kasa_banka_id,olusturma_tarihi_saat,olusturan_kullanici_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                               (tarih, cari_tip_str, cari_id, islem_tipi_ch_pesin, tutar, aciklama_pesin, fatura_id, referans_tip_pesin, kasa_banka_id, current_time, olusturan_id))

    def toplu_stok_analiz_et(self, stok_data_raw, selected_update_fields):
        analysis_results = {
            'new_items': [], 'updated_items': [], 'errors_details': [],
            'new_count': 0, 'updated_count': 0, 'error_count': 0,
            'all_processed_data': [], 'selected_update_fields_from_ui': selected_update_fields
        }
        ...
        try:
            ...
            for i, row_raw in enumerate(stok_data_raw):
                ...
                try:
                    ...
                    processed_data_item = {
                        "urun_id": existing_product['id'] if existing_product else None,
                        "urun_kodu": urun_kodu,
                        "urun_adi": urun_adi_from_excel,
                        "stok_miktari": self.safe_float(row[COL_STOK_MIKTARI]),
                        "alis_fiyati_kdv_dahil": self.safe_float(row[COL_ALIS_FIYATI_KDV_DAHIL]),
                        "satis_fiyati_kdv_dahil": self.safe_float(row[COL_SATIS_FIYATI_KDV_DAHIL]),
                        "kdv_orani": self.safe_float(row[COL_KDV_ORANI]),
                        "min_stok_seviyesi": self.safe_float(row[COL_MIN_STOK_SEVIYESI]),
                        "urun_detayi": str(row[COL_URUN_DETAYI] or '').strip(),
                        "urun_resmi_yolu": str(row[COL_URUN_RESMI_YOLU] or '').strip(),
                        "kategori_id_or_name": final_kategori,
                        "marka_id_or_name": final_marka,
                        "urun_grubu_id_or_name": final_urun_grubu,
                        "urun_birimi_id_or_name": final_urun_birimi,
                        "ulke_id_or_name": final_ulke,
                        "initial_stok_db": existing_product['stok_miktari'] if existing_product else 0.0
                    }
                    ...
                    analysis_results['errors_details'].append((row_raw, f"Satır işlenirken hata: {e_inner_loop}."))
                    analysis_results['error_count'] += 1

        except Exception as e:
            analysis_results['errors_details'].append( (None, f"Analiz sırasında beklenmeyen bir hata oluştu: {e}. {traceback.format_exc()}") )
            analysis_results['error_count'] += 1
        finally:
            if conn_thread:
                conn_thread.close()
    ...
    def toplu_musteri_analiz_et(self, musteriler_data_raw):
        ...
        COL_MUSTERI_KODU, COL_AD_SOYAD = 0, 1
        COL_TELEFON, COL_ADRES, COL_VERGI_DAIRESI, COL_VERGI_NO = 2, 3, 4, 5
        expected_min_excel_columns = 2
        ...
        try:
            ...
            for i, row_raw in enumerate(musteriler_data_raw):
                ...
                if len(row_raw) < expected_min_excel_columns:
                    analysis_results['errors_details'].append((row_raw, f"Eksik temel sütunlar. En az {expected_min_excel_columns} sütun bekleniyor."))
                    analysis_results['error_count'] += 1
                    continue

                try:
                    musteri_kodu = str(row[COL_MUSTERI_KODU] or '').strip()
                    ad = str(row[COL_AD_SOYAD] or '').strip()

                    if not musteri_kodu or not ad:
                        analysis_results['errors_details'].append((row_raw, "Müşteri Kodu veya Adı boş olamaz."))
                        analysis_results['error_count'] += 1
                        continue
                    if musteri_kodu == self.PERAKENDE_MUSTERI_KODU:
                        analysis_results['errors_details'].append((row_raw, f"'{self.PERAKENDE_MUSTERI_KODU}' özel bir koddur."))
                        analysis_results['error_count'] += 1
                        continue

                    cursor_thread.execute("SELECT id FROM musteriler WHERE musteri_kodu=?", (musteri_kodu,))
                    existing_customer = cursor_thread.fetchone()

                    status_message = "Yeni müşteri eklenecek."
                    if existing_customer:
                        status_message = "Mevcut müşteri güncellenecek."
                        analysis_results['updated_items'].append((row_raw, status_message))
                        analysis_results['updated_count'] += 1
                    else:
                        analysis_results['new_items'].append((row_raw, status_message))
                    ...
                    analysis_results['all_processed_data'].append(row_raw)
    ...
    def toplu_tedarikci_analiz_et(self, tedarikciler_data_raw):
        ...
        COL_TEDARIKCI_KODU, COL_AD_SOYAD = 0, 1
        COL_TELEFON, COL_ADRES, COL_VERGI_DAIRESI, COL_VERGI_NO = 2, 3, 4, 5
        expected_min_excel_columns = 2
        ...
        try:
            ...
            for i, row_raw in enumerate(tedarikciler_data_raw):
                row = list(row_raw) + [None] * (COL_VERGI_NO + 1 - len(row_raw))
                ...
                try:
                    tedarikci_kodu = str(row[COL_TEDARIKCI_KODU] or '').strip()
                    ad = str(row[COL_AD_SOYAD] or '').strip()

                    if not tedarikci_kodu or not ad:
                        analysis_results['errors_details'].append((row_raw, "Tedarikçi Kodu veya Adı boş olamaz."))
                        analysis_results['error_count'] += 1
                        continue

                    cursor_thread.execute("SELECT id FROM tedarikciler WHERE tedarikci_kodu=?", (tedarikci_kodu,))
                    existing_supplier = cursor_thread.fetchone()

                    status_message = "Yeni tedarikçi eklenecek."
                    if existing_supplier:
                        status_message = "Mevcut tedarikçi güncellenecek."
                        analysis_results['updated_items'].append((row_raw, status_message))
                        analysis_results['updated_count'] += 1
                    else:
                        analysis_results['new_items'].append((row_raw, status_message))
    ...
    def toplu_stok_ekle_guncelle(self, processed_stok_data, selected_update_fields_from_analysis):
        """
        Excel'den gelen veriyi işler. Ürün kartlarını ekler veya SADECE SEÇİLEN ALANLARI günceller.
        Excel'deki boş hücreler, mevcut veriyi ezmez. Ürün birimi boş ise 'Adet' varsayılır.
        Stok artışları için tek bir toplu alış faturası oluşturur.
        """
        ...
        try:
            ...
            for item_data in processed_stok_data:
                try:
                    # <<< YENİ YAZMA MANTIĞI >>>
                    # Nitelik ID'lerini çözme (isim gelmişse ID'ye dönüştür, yoksa None)
                    def resolve_id(key, table, column, default_if_empty=None):
                        value = item_data.get(key)
                        if isinstance(value, int): return value
                        if isinstance(value, str) and value:
                            return self._get_or_create_id(table, column, value, manage_transaction=False)
                        if default_if_empty:
                             return self._get_or_create_id(table, column, default_if_empty, manage_transaction=False)
                        return None
                    ...
                    alis_haric = alis_dahil / kdv_carpan if kdv_carpan != 0 else alis_dahil
                    satis_haric = satis_dahil / kdv_carpan if kdv_carpan != 0 else satis_dahil
                    
                    urun_id = item_data.get('urun_id')
                    if urun_id is not None:  # --- GÜNCELLEME MANTIĞI ---
                        updated_product_count_final += 1
                        
                        update_clauses = []
                        update_params = []
                        
                        # Ürün adı ve detayı her zaman güncellenir (Excel'de varsa)
                        if item_data.get('urun_adi'): 
                            update_clauses.append("urun_adi=?")
                            update_params.append(item_data.get('urun_adi'))
                        if item_data.get('urun_detayi'): 
                            update_clauses.append("urun_detayi=?")
                            update_params.append(item_data.get('urun_detayi'))

                        if 'fiyat_bilgileri' in selected_update_fields_from_analysis:
                            if item_data.get('alis_fiyati_kdv_dahil') is not None: 
                                update_clauses.extend(["alis_fiyati_kdv_haric=?", "alis_fiyati_kdv_dahil=?"]); update_params.extend([alis_haric, alis_dahil])
                            if item_data.get('satis_fiyati_kdv_dahil') is not None: 
                                update_clauses.extend(["satis_fiyati_kdv_haric=?", "satis_fiyati_kdv_dahil=?"]); update_params.extend([satis_haric, satis_dahil])
                            if kdv_orani is not None: 
                                update_clauses.append("kdv_orani=?"); update_params.append(kdv_orani)
                        ...
                        if 'stok_miktari' in selected_update_fields_from_analysis:
                            if item_data.get('min_stok_seviyesi') is not None: 
                                update_clauses.append("min_stok_seviyesi=?"); update_params.append(item_data.get('min_stok_seviyesi'))
                        
                        if update_clauses:
                            update_clauses.extend(["son_guncelleme_tarihi_saat=?", "son_guncelleyen_kullanici_id=?"])
                            update_params.extend([current_time, olusturan_id])
                            update_params.append(urun_id)
                            
                            sql_query = f"UPDATE tbl_stoklar SET {', '.join(update_clauses)} WHERE id=?"
                            self.c.execute(sql_query, tuple(update_params))

                    else:  # --- YENİ ÜRÜN EKLEME MANTIĞI ---
                        new_product_count_final += 1
                        self.c.execute("""
                            INSERT INTO tbl_stoklar (urun_kodu, urun_adi, stok_miktari, alis_fiyati_kdv_haric, satis_fiyati_kdv_haric, 
                            kdv_orani, min_stok_seviyesi, alis_fiyati_kdv_dahil, satis_fiyati_kdv_dahil, kategori_id, marka_id, 
                            urun_grubu_id, urun_birimi_id, ulke_id, urun_detayi, urun_resmi_yolu, olusturma_tarihi_saat, olusturan_kullanici_id)
                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                        """, (item_data.get('urun_kodu'), item_data.get('urun_adi'), 0.0, alis_haric, satis_haric, kdv_orani, 
                              item_data.get('min_stok_seviyesi', 0.0), alis_dahil, satis_dahil, kategori_id_final, marka_id_final,
                              urun_grubu_id_final, urun_birimi_id_final, ulke_id_final, item_data.get('urun_detayi'),
                              item_data.get('urun_resmi_yolu'), current_time, olusturan_id))
                        urun_id = self.c.lastrowid
                    
                    if 'stok_miktari' in selected_update_fields_from_analysis and item_data.get('stok_miktari') is not None:
                        stok_farki = item_data.get('stok_miktari', 0.0) - item_data.get('initial_stok_db', 0.0)
                        if stok_farki > 0:
                            kalemler_for_fatura.append((urun_id, stok_farki, alis_haric, kdv_orani, alis_dahil, 0, 0, "YOK", 0))

                except Exception as e_inner:
                    total_errors_during_write.append(f"Ürün Kodu {item_data.get('urun_kodu')}: {e_inner}")

            if kalemler_for_fatura:
                fatura_no_aktarim = f"AKT-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                if self.genel_tedarikci_id is None: raise Exception("Genel tedarikçi bulunamadı.")
                success_fatura, _ = self.fatura_olustur(fatura_no_aktarim, 'ALIŞ', self.genel_tedarikci_id,
            ...
            if total_errors_during_write: message += f"\n{len(total_errors_during_write)} satırda hata oluştu: {'; '.join(total_errors_during_write[:2])}"
            return True, message

        except Exception as e_main:
            self.conn.rollback()
            return False, f"Toplu işlem sırasında kritik hata: {e_main}\n{traceback.format_exc()}"
    ...
    def siparis_faturaya_donustur(self, siparis_id, olusturan_kullanici_id, odeme_turu_secilen=None, kasa_banka_id_secilen=None, vade_tarihi_secilen=None): 
        """
        Belirtilen siparişi bir faturaya dönüştürür.
        Sipariş kalemlerini alır, yeni bir fatura oluşturur ve
        siparişin durumunu 'TAMAMLANDI' olarak günceller ve fatura_id'sini kaydeder.
        Dönüş: (bool, mesaj)
        """
        try:
            self.conn.execute("BEGIN TRANSACTION")
            siparis_ana = self.get_siparis_by_id(siparis_id)
            if not siparis_ana:
                return False, "Dönüştürülecek sipariş bulunamadı."
            
            _id, s_no_local, tarih_db, cari_tip_db, cari_id_db, toplam_tutar_db, durum_db, fatura_id_ref_db, \
            olusturma_tarihi_saat, olusturan_kullanici_id_orig, son_guncelleme_tarihi_saat, \
            son_guncelleyen_kullanici_id, siparis_notlari_db, onay_durumu_db, teslimat_tarihi_db, \
            genel_iskonto_tipi_db, genel_iskonto_degeri_db = siparis_ana

            if fatura_id_ref_db:
                return False, f"Bu sipariş zaten '{fatura_id_ref_db}' nolu bir faturaya dönüştürülmüş."
            if durum_db == 'İPTAL EDİLDİ':
                return False, "İptal edilmiş bir sipariş faturaya dönüştürülemez."

            siparis_kalemleri = self.get_siparis_kalemleri(siparis_id)
            if not siparis_kalemleri:
                return False, "Sipariş kalemleri bulunamadı. Fatura oluşturulamıyor."

            # Fatura tipi ve fatura no belirleme
            fatura_tipi = 'SATIŞ' if cari_tip_db == 'MUSTERI' else 'ALIŞ'
            fatura_no_prefix = "SAT" if fatura_tipi == 'SATIŞ' else "AL"
            fatura_no = f"{fatura_no_prefix}-{datetime.now().strftime('%Y%m%d%H%M%S%f')}"

            kalemler_for_fatura_olustur = []
            for sk in siparis_kalemleri:
                urun_id_sk = sk[2]
                miktar_sk = sk[3]
                birim_fiyat_kdv_haric_sk = sk[4]
                kdv_orani_sk = sk[5]
                alis_fiyati_siparis_aninda_sk = sk[9]
                satis_fiyati_siparis_aninda_sk = sk[10]
                iskonto_yuzde_1_sk = sk[11]
                iskonto_yuzde_2_sk = sk[12]
                alis_fiyati_fatura_aninda_for_fatura_kalem = alis_fiyati_siparis_aninda_sk
                
                kalemler_for_fatura_olustur.append((
                    urun_id_sk, miktar_sk, birim_fiyat_kdv_haric_sk, kdv_orani_sk, 
                    alis_fiyati_fatura_aninda_for_fatura_kalem,
                    iskonto_yuzde_1_sk, iskonto_yuzde_2_sk,
                    "YOK", 0.0
                ))

            # Fatura oluşturma işlemini çağır
            success_fatura, message_fatura_id = self.fatura_olustur( 
                fatura_no, fatura_tipi, cari_id_db, kalemler_for_fatura_olustur,
                odeme_turu=odeme_turu_secilen,
                kasa_banka_id=kasa_banka_id_secilen,
                fatura_notlari=f"Sipariş No: {s_no_local} ile oluşturulmuştur. {siparis_notlari_db or ''}".strip(),
                vade_tarihi=vade_tarihi_secilen,
                genel_iskonto_tipi=genel_iskonto_tipi_db,
                genel_iskonto_degeri=genel_iskonto_degeri_db,
                manage_transaction=False # Ana transaction'ı bu metot yönetecek
            )
            ...
            if success_fatura:
                ...
                # Siparişin durumunu güncelle ve fatura_id'sini kaydet
                current_time = self.get_current_datetime_str()
                self.c.execute("""
                    UPDATE siparisler SET durum=?, fatura_id=?, son_guncelleme_tarihi_saat=?, son_guncelleyen_kullanici_id=?
                    WHERE id=?
                """, ('TAMAMLANDI', yeni_fatura_id, current_time, olusturan_kullanici_id, siparis_id))
                self.conn.commit()

                return True, f"Sipariş '{s_no_local}' başarıyla '{yeni_fatura_id}' nolu faturaya dönüştürüldü."
            else:
                self.conn.rollback()
    ...
    def get_siparisler_by_cari(self, cari_tip, cari_id):
        ...
        """
        try:
            # DİKKAT: cari_tip burada 'MUSTERI' veya 'TEDARIKCI' olmalıdır.
            query = """
                SELECT 
                    s.id, 
                    s.siparis_no, 
    ...
    def fatura_guncelle(self, fatura_id, yeni_fatura_no, yeni_cari_id, yeni_odeme_turu, yeni_kalemler, guncelle_alis_fiyatlari, kasa_banka_id=None, yeni_misafir_adi=None, yeni_fatura_notlari=None, yeni_vade_tarihi=None, genel_iskonto_tipi='YOK', genel_iskonto_degeri=0.0):
        try:
            self.conn.execute("BEGIN TRANSACTION")

            eski_fatura_bilgisi = self.fatura_getir_by_id(fatura_id)
            if not eski_fatura_bilgisi:
                self.conn.rollback()
                return False, "Güncellenecek fatura bulunamadı."
    
            eski_kalemler = self.fatura_detay_al(fatura_id)
            fatura_tipi = eski_fatura_bilgisi['tip']
            tarih_str = eski_fatura_bilgisi['tarih']

            # Eski faturanın finansal ve stok etkilerini geri al
            self._fatura_finansal_etki_sil(fatura_id)
            
            for kalem in eski_kalemler:
                urun_id, miktar = kalem['urun_id'], kalem['miktar']
                
                # <<< DÜZELTME 1: Hatalı olan "self.db" çağrısı "self" olarak düzeltildi. >>>
                stok_oncesi_tuple = self.stok_getir_by_id(urun_id)
                if not stok_oncesi_tuple: continue
                
                if fatura_tipi == 'SATIŞ':
                    self.c.execute("UPDATE tbl_stoklar SET stok_miktari = stok_miktari + ? WHERE id=?", (miktar, urun_id))
                elif fatura_tipi == 'ALIŞ':
                    self.c.execute("UPDATE tbl_stoklar SET stok_miktari = stok_miktari - ? WHERE id=?", (miktar, urun_id))
                    
            self.c.execute("DELETE FROM stok_hareketleri WHERE kaynak='FATURA' AND kaynak_id=?", (fatura_id,))
            self.c.execute("DELETE FROM fatura_kalemleri WHERE fatura_id=?", (fatura_id,))

            # ... (Metodun geri kalanı aynı şekilde devam ediyor)
            # Adım 3: Yeni kalemlere göre yeni toplamları hesapla
            toplam_kdv_haric_kalemler, toplam_kdv_dahil_kalemler = 0.0, 0.0
            for urun_id, miktar_val, birim_fiyat_kdv_haric_val, kdv_orani_val, alis_fiyati_fatura_aninda_val, iskonto_yuzde_1_val, iskonto_yuzde_2_val, _, _ in yeni_kalemler:
                miktar_f = self.safe_float(miktar_val)
                birim_fiyat_kdv_haric_f = self.safe_float(birim_fiyat_kdv_haric_val)
                kdv_orani_f = self.safe_float(kdv_orani_val)
                orijinal_birim_fiyat_kdv_dahil = birim_fiyat_kdv_haric_f * (1 + kdv_orani_f / 100)
                fiyat_iskonto_1_sonrasi_dahil = orijinal_birim_fiyat_kdv_dahil * (1 - self.safe_float(iskonto_yuzde_1_val) / 100)
                iskontolu_birim_fiyat_dahil = fiyat_iskonto_1_sonrasi_dahil * (1 - self.safe_float(iskonto_yuzde_2_val) / 100)
                if kdv_orani_f == 0: iskontolu_birim_fiyat_haric = iskontolu_birim_fiyat_dahil
                else: iskontolu_birim_fiyat_haric = iskontolu_birim_fiyat_dahil / (1 + kdv_orani_f / 100)
                toplam_kdv_haric_kalemler += miktar_f * iskontolu_birim_fiyat_haric
                toplam_kdv_dahil_kalemler += miktar_f * iskontolu_birim_fiyat_dahil
            ...
            # Adım 4: Ana fatura kaydını güncelle
            current_time = self.get_current_datetime_str()
            guncelleyen_id = self.app.current_user[0] if self.app and self.app.current_user else None

            self.c.execute("""
                UPDATE faturalar SET
                fatura_no=?, cari_id=?, toplam_kdv_haric=?, toplam_kdv_dahil=?, odeme_turu=?,
                misafir_adi=?, kasa_banka_id=?, fatura_notlari=?, vade_tarihi=?,
                genel_iskonto_tipi=?, genel_iskonto_degeri=?,
                son_guncelleme_tarihi_saat=?, son_guncelleyen_kullanici_id=?
                WHERE id=?
            """, (yeni_fatura_no, yeni_cari_id, nihai_toplam_kdv_haric_fatura, nihai_toplam_kdv_dahil_fatura, yeni_odeme_turu,
                  yeni_misafir_adi, kasa_banka_id, yeni_fatura_notlari, yeni_vade_tarihi,
                  genel_iskonto_tipi, genel_iskonto_degeri,
                  current_time, guncelleyen_id, fatura_id))

            # Adım 5: Yeni kalemleri ve finansal/stok etkilerini oluştur
            for urun_id, miktar_val, birim_fiyat_kdv_haric_val, kdv_orani_val, alis_fiyati_fatura_aninda_val, iskonto_yuzde_1_val, iskonto_yuzde_2_val, iskonto_tipi_genel_val, iskonto_degeri_genel_val in yeni_kalemler:
                # ... (kalem hesaplamaları aynı) ...
                miktar_f = self.safe_float(miktar_val)
                birim_fiyat_kdv_haric_f = self.safe_float(birim_fiyat_kdv_haric_val)
                kdv_orani_f = self.safe_float(kdv_orani_val)
                alis_fiyati_fatura_aninda_f = self.safe_float(alis_fiyati_fatura_aninda_val)
                ...
                self.c.execute("INSERT INTO fatura_kalemleri (fatura_id, urun_id, miktar, birim_fiyat, kdv_orani, kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_fatura_aninda, iskonto_yuzde_1, iskonto_yuzde_2, iskonto_tipi, iskonto_degeri, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               (fatura_id, urun_id, miktar_f, birim_fiyat_kdv_haric_f, kdv_orani_f, kalem_kdv_tutari, kalem_toplam_kdv_haric, kalem_toplam_kdv_dahil, alis_fiyati_fatura_aninda_f, iskonto_yuzde_1_f, iskonto_yuzde_2_f, iskonto_tipi_genel_val, iskonto_degeri_genel_val, current_time, guncelleyen_id))
                
                self._stok_guncelle_ve_hareket_kaydet('FATURA', fatura_id, urun_id, miktar_f, fatura_tipi, yeni_fatura_no)
            
            is_perakende_satis = (fatura_tipi == 'SATIŞ' and self.perakende_musteri_id is not None and str(yeni_cari_id) == str(self.perakende_musteri_id))
            self._fatura_finansal_etki_olustur(fatura_id, yeni_fatura_no, fatura_tipi, tarih_str, yeni_cari_id, nihai_toplam_kdv_dahil_fatura, yeni_odeme_turu, kasa_banka_id, is_perakende_satis)
            ...
            self.conn.rollback()
            error_details = traceback.format_exc()
            logging.error(f"Fatura güncellenirken kritik bir hata oluştu: {e}\nDetaylar: {error_details}")
            return False, f"Fatura güncellenirken kritik bir hata oluştu. Detaylar için log dosyasına bakınız."
    ...
    def fatura_listele_urun_ad_dahil(self, tip=None, baslangic_tarih=None, bitis_tarih=None, arama_terimi=None, cari_id_filter=None, odeme_turu_filter=None, kasa_banka_id_filter=None, limit=None, offset=None):
        # Perakende müşteri ID'si her zaman ilk parametre olarak kullanılacak.
        perakende_id_param = self.perakende_musteri_id if self.perakende_musteri_id is not None else -999

        q = """SELECT f.id, f.fatura_no, f.tarih, f.tip,
                       CASE
                           WHEN f.cari_id = ? AND f.tip = 'SATIŞ' THEN IFNULL(f.misafir_adi, 'Perakende Satış') -- Parametre 1
                           WHEN f.tip = 'ALIŞ' THEN ted.ad
                           WHEN f.tip = 'SATIŞ' THEN mus.ad
                           ELSE 'Bilinmeyen Cari'
                       END AS cari_adi,
                       ...
                       f.vade_tarihi,                 -- <-- BURADA
                       f.genel_iskonto_degeri,        -- <-- BURADA
                       olusturan.kullanici_adi AS olusturan_kul_adi,   -- <-- BURADA
                       guncelleyen.kullanici_adi AS guncelleyen_kul_adi -- <-- BURADA
               FROM faturalar f
               ...
               """
        # SQL sorgusunun başlangıç parametreleri (CASE ifadesi için)
        ...
        if arama_terimi:
            ...
            conditions.append("""(
                                f.fatura_no LIKE ? OR
                                (f.tip = 'SATIŞ' AND IFNULL(f.misafir_adi, 'Perakende Satış') LIKE ?) OR -- Perakende için arama
                                (f.tip = 'SATIŞ' AND mus.ad LIKE ?) OR
                                (f.tip = 'ALIŞ' AND ted.ad LIKE ?) OR
                                kb.hesap_adi LIKE ? OR
                                EXISTS (SELECT 1 FROM fatura_kalemleri fk JOIN tbl_stoklar s ON fk.urun_id = s.id WHERE fk.fatura_id = f.id AND (s.urun_adi LIKE ? OR s.urun_kodu LIKE ?))
                              )""")
            # Parametreler sırasıyla:
            # f.fatura_no LIKE ?                                 -> term
            # IFNULL(f.misafir_adi, 'Perakende Satış') LIKE ?    -> term
            # mus.ad LIKE ?                                      -> term
            # ted.ad LIKE ?                                      -> term
            # kb.hesap_adi LIKE ?                                -> term
            # s.urun_adi LIKE ?                                  -> term
            # s.urun_kodu LIKE ?                                 -> term
        ...
        # Debugging için: Oluşturulan sorguyu ve parametreleri yazdırın.
        print(f"DEBUG SQL Query: {q}")
        print(f"DEBUG SQL Params: {p}")
    ...
    def get_fatura_count(self, tip=None, baslangic_tarih=None, bitis_tarih=None, arama_terimi=None, cari_id_filter=None, odeme_turu_filter=None, kasa_banka_id_filter=None):
        ...
        p = []  # Parametre listesini boş başlatıyoruz
        conditions = []

        # DÜZELTME BAŞLANGICI: Perakende müşteri için özel koşulu conditions listesine ekliyoruz
        # Bu koşul sadece SATIŞ faturaları için geçerli ve cari_adi'nı doğru saymak için önemli.
        # Bu, get_fatura_listele_urun_ad_dahil metodundaki CASE ifadesinin WHERE karşılığıdır.
        conditions.append(" ( (f.tip = 'SATIŞ' AND f.cari_id = ? AND IFNULL(f.misafir_adi, 'Perakende Satış') IS NOT NULL) OR (f.tip = 'SATIŞ' AND f.cari_id != ?) OR (f.tip = 'ALIŞ') ) ")
        ...
        if arama_terimi:
            ...
            conditions.append("""(
                                f.fatura_no LIKE ? OR
                                (f.tip = 'SATIŞ' AND IFNULL(f.misafir_adi, 'Perakende Satış') LIKE ?) OR
                                (f.tip = 'SATIŞ' AND mus.ad LIKE ?) OR
                                (f.tip = 'ALIŞ' AND ted.ad LIKE ?) OR
                                kb.hesap_adi LIKE ? OR
                                s.urun_adi LIKE ? OR s.urun_kodu LIKE ?
                              )""")
            p.extend([term, term, term, term, term, term, term])
    ...
    def fatura_sil(self, fatura_id, manage_transaction=True):
        """
        Bir faturayı ve onunla ilişkili tüm kayıtları (stok hareketleri,
        finansal hareketler vb.) doğru bir şekilde siler ve stokları günceller.
        EĞER FATURA BİR SİPARİŞTEN OLUŞTURULDUYSA, SİPARİŞİN DURUMUNU GÜNCELLER.
        """
        try:
            fatura_bilgisi = self.fatura_getir_by_id(fatura_id)
            if not fatura_bilgisi:
                return False, "Silinecek fatura bulunamadı."
            ...
            kalemler = self.fatura_detay_al(fatura_id)

            # Bu faturaya bağlı bir sipariş var mı diye kontrol et
            self.c.execute("SELECT id FROM siparisler WHERE fatura_id = ?", (fatura_id,))
            ilgili_siparis = self.c.fetchone()

            if ilgili_siparis:
                # Eğer varsa, siparişin durumunu 'BEKLEMEDE' yap ve fatura bağlantısını kaldır.
                ilgili_siparis_id = ilgili_siparis['id']
                self.c.execute("UPDATE siparisler SET durum = 'BEKLEMEDE', fatura_id = NULL, son_guncelleme_tarihi_saat = ?, son_guncelleyen_kullanici_id = ? WHERE id = ?",
                               (self.get_current_datetime_str(), self.app.current_user[0] if self.app and self.app.current_user else None, ilgili_siparis_id))
                print(f"DEBUG: Fatura ID {fatura_id} silinirken, ilişkili Sipariş ID {ilgili_siparis_id} güncellendi.")

            # 1. Adım: Stok Miktarlarını Tersine Çevir
            ...
            # 2. Adım: Tüm İlişkili Kayıtları Sil
            self._fatura_finansal_etki_sil(fatura_id)
            self.c.execute("DELETE FROM stok_hareketleri WHERE kaynak='FATURA' AND kaynak_id=?", (fatura_id,))
            self.c.execute("DELETE FROM fatura_kalemleri WHERE fatura_id=?", (fatura_id,))
            self.c.execute("DELETE FROM faturalar WHERE id=?", (fatura_id,))

            if manage_transaction:
                self.conn.commit()

            return True, f"Fatura No '{fatura_no_silinen}' başarıyla silindi ve stoklar güncellendi."
        except Exception as e:
            if manage_transaction:
                self.conn.rollback()
            error_details = traceback.format_exc()
            logging.error(f"Fatura silinirken hata oluştu: {e}\nDetaylar: {error_details}")
            return False, "Fatura silinirken beklenmeyen bir hata oluştu."
    
    def _fatura_finansal_etki_sil(self, fatura_id):
        """Bir fatura silindiğinde veya güncellendiğinde finansal etkileri geri alır."""
        fatura_bilgisi = self.fatura_getir_by_id(fatura_id)
        if not fatura_bilgisi:
            return

        odeme_turu = fatura_bilgisi['odeme_turu']
        kasa_banka_id = fatura_bilgisi['kasa_banka_id']
        tutar = fatura_bilgisi['toplam_kdv_dahil']
        fatura_tipi = fatura_bilgisi['tip']

        # Eğer peşin bir işlemse, Kasa/Banka bakiyesini tersine çevir
        if odeme_turu in self.pesin_odeme_turleri and kasa_banka_id is not None:
            is_ters_bakiye_artir = (fatura_tipi != 'SATIŞ') # Satış silinirse bakiye azalır (artir=False), Alış silinirse artar (artir=True)
            self.kasa_banka_bakiye_guncelle(kasa_banka_id, tutar, artir=is_ters_bakiye_artir)

        # Faturaya bağlı tüm gelir/gider ve cari hareket kayıtlarını sil
        self.c.execute("DELETE FROM gelir_gider WHERE kaynak='FATURA' AND kaynak_id=?", (fatura_id,))
        self.c.execute("DELETE FROM cari_hareketler WHERE referans_id=? AND referans_tip LIKE 'FATURA%'", (fatura_id,))
    ...
    def gelir_gider_ekle(self, tarih, tip, tutar, aciklama, kasa_banka_id=None, gelir_siniflandirma_id=None, gider_siniflandirma_id=None): # <-- DÜZELTME: Yeni parametreler eklendi
        ...
        try:
            ...
            current_time = self.get_current_datetime_str()
            olusturan_id = self.app.current_user[0] if self.app and self.app.current_user else None
            # DÜZELTME BAŞLANGICI: INSERT sorgusuna yeni sütunlar ve değerler eklendi
            self.c.execute("INSERT INTO gelir_gider (tarih, tip, tutar, aciklama, kaynak, kasa_banka_id, gelir_siniflandirma_id, gider_siniflandirma_id, olusturma_tarihi_saat, olusturan_kullanici_id) VALUES (?,?,?,?, 'MANUEL', ?, ?, ?, ?, ?)",
                           (tarih, tip, tutar_f, aciklama, kasa_banka_id, gelir_siniflandirma_id, gider_siniflandirma_id, current_time, olusturan_id))
    ...
    def get_tedarikci_net_bakiye(self, tedarikci_id):
        """
        Tedarikçiye olan net VERESİYE borcumuzu hesaplar. Peşin faturalar bu hesaba dahil edilmez.
        Pozitif değer: Bizim tedarikçiye borcumuz var.
        Negatif değer: Tedarikçinin bize borcu var.
        """
        # Tedarikçiye olan veresiye borcumuz (Sadece Açık Hesap Faturalar ve Manuel Borçlar)
        query_borc = """
            SELECT SUM(ch.tutar) FROM cari_hareketler ch
            LEFT JOIN faturalar f ON ch.referans_id = f.id AND ch.referans_tip = 'FATURA'
            WHERE ch.cari_id = ? AND ch.cari_tip = 'TEDARIKCI'
            AND (
                (ch.referans_tip = 'FATURA' AND f.odeme_turu = 'AÇIK HESAP') OR
                ch.referans_tip = 'VERESIYE_BORC_MANUEL'
            )
        """
        self.c.execute(query_borc, (tedarikci_id,))
        toplam_borc = self.c.fetchone()[0] or 0.0

        # <<< DÜZELTME BURADA >>>
        # Tedarikçiye yapılan ödemeleri alırken, peşin fatura ödemelerini DAHİL ETME.
        query_odeme = """
            SELECT SUM(tutar) FROM cari_hareketler
            WHERE cari_id = ? AND cari_tip = 'TEDARIKCI' AND islem_tipi = 'ODEME'
            AND referans_tip != 'FATURA_ALIS_PESIN'
        """
        self.c.execute(query_odeme, (tedarikci_id,))
        toplam_odeme = self.c.fetchone()[0] or 0.0

        net_bakiye = toplam_borc - toplam_odeme
        return net_bakiye

    def get_musteri_net_bakiye(self, musteri_id):
        # Müşterinin veresiye borçları (Sadece Açık Hesap Faturalar ve Manuel Borçlar)
        query_alacak = """
            SELECT SUM(ch.tutar) FROM cari_hareketler ch
            LEFT JOIN faturalar f ON ch.referans_id = f.id AND ch.referans_tip = 'FATURA'
            WHERE ch.cari_id = ? AND ch.cari_tip = 'MUSTERI' AND ch.islem_tipi = 'ALACAK'
            AND (f.odeme_turu = 'AÇIK HESAP' OR ch.referans_tip = 'VERESIYE_BORC_MANUEL')
        """
        self.c.execute(query_alacak, (musteri_id,))
        toplam_alacak = self.c.fetchone()[0] or 0.0

        # Müşteriden yapılan tahsilatları alırken, peşin fatura tahsilatlarını DAHİL ETME.
        query_tahsilat = """
            SELECT SUM(tutar) FROM cari_hareketler
            WHERE cari_id = ? AND cari_tip = 'MUSTERI' AND islem_tipi = 'TAHSILAT'
            AND referans_tip != 'FATURA_SATIS_PESIN'
        """
        self.c.execute(query_tahsilat, (musteri_id,))
        toplam_tahsilat = self.c.fetchone()[0] or 0.0

        net_bakiye = toplam_alacak - toplam_tahsilat
        return net_bakiye

    def _get_cari_devir_bakiye(self, cari_tip, cari_id, baslangic_tarih_str):
        """
        Belirtilen bir başlangıç tarihinden önceki tüm hareketleri hesaplayarak
        devir bakiyesini döndürür.
        """
        devir_bakiye = 0.0
        try:
            query = """
                    SELECT islem_tipi, tutar, referans_tip FROM cari_hareketler -- referans_tip'i de seçiyoruz
    ...
    def cari_hesap_ekstresi_al(self, cari_tip, cari_id, baslangic_tarih_str, bitis_tarih_str):
        logging.debug(f"cari_hesap_ekstresi_al çağrıldı. Cari Tip: {cari_tip}, Cari ID: {cari_id}, Başlangıç: {baslangic_tarih_str}, Bitiş: {bitis_tarih_str}")
        
        try:
            devreden_bakiye = self._get_cari_devir_bakiye(cari_tip, cari_id, baslangic_tarih_str)

            # <<< DÜZELTME: Sorguya f.odeme_turu sütununu ekledik ve peşin işlemleri daha iyi etiketlemek için referans tipini de alıyoruz >>>
            query = """
                SELECT
            ...
            """
            params = [cari_tip, cari_id, baslangic_tarih_str, bitis_tarih_str]
            
            self.c.execute(query, params)
            hareketler_listesi = self.c.fetchall()
            
            logging.debug(f"Veritabanından {len(hareketler_listesi)} adet cari hareket kaydı bulundu.")

            # Bu kısım artık kullanılmıyor, bakiye hesaplaması arayüzde yapılacak.
            toplam_borc_donem = 0.0
            toplam_alacak_donem = 0.0
            
            return hareketler_listesi, devreden_bakiye, toplam_borc_donem, toplam_alacak_donem
        
        except Exception as e:
            logging.error(f"cari_hesap_ekstresi_al içinde genel hata: {e}\n{traceback.format_exc()}")
            return [], 0.0, 0.0, 0.0

    def get_cari_ozet_bilgileri(self, cari_id, cari_tip):
        ozet = { "toplam_satis": 0.0, "toplam_alis": 0.0, "toplam_tahsilat": 0.0, "toplam_odeme": 0.0, "net_bakiye": 0.0 }
        try:
            if cari_tip == 'MUSTERI':
                # Toplam satışı, hem peşin hem veresiye tüm faturalardan al.
                self.c.execute("SELECT SUM(toplam_kdv_dahil) FROM faturalar WHERE tip='SATIŞ' AND cari_id=?", (cari_id,))
                ozet["toplam_satis"] = self.c.fetchone()[0] or 0.0
                
                # <<< YENİ VE DOĞRU TAHSİLAT HESAPLAMA MANTIĞI >>>
                # Toplam tahsilatı, SADECE cari hareketler tablosundaki 'TAHSILAT' işlemlerinden alıyoruz.
                # Bu, hem manuel tahsilatları hem de peşin faturaların tahsilatını TEK SEFERDE doğru toplar.
                self.c.execute("SELECT SUM(tutar) FROM cari_hareketler WHERE cari_tip='MUSTERI' AND cari_id=? AND islem_tipi='TAHSILAT'", (cari_id,))
                ozet["toplam_tahsilat"] = self.c.fetchone()[0] or 0.0
                
                # Net bakiyeyi hesapla (bu metot sadece veresiye işlemleri baz aldığı için zaten doğru).
                ozet["net_bakiye"] = self.get_musteri_net_bakiye(cari_id)

            elif cari_tip == 'TEDARIKCI':
                # Tedarikçi için de aynı mantığı uygula
                self.c.execute("SELECT SUM(toplam_kdv_dahil) FROM faturalar WHERE tip='ALIŞ' AND cari_id=?", (cari_id,))
                ozet["toplam_alis"] = self.c.fetchone()[0] or 0.0
                
                # Toplam ödemeyi SADECE cari hareketlerdeki 'ODEME' kayıtlarından al.
                self.c.execute("SELECT SUM(tutar) FROM cari_hareketler WHERE cari_tip='TEDARIKCI' AND cari_id=? AND islem_tipi='ODEME'", (cari_id,))
                ozet["toplam_odeme"] = self.c.fetchone()[0] or 0.0

                ozet["net_bakiye"] = self.get_tedarikci_net_bakiye(cari_id)
    ...
    def kasa_banka_bakiye_guncelle(self, kasa_banka_id, islem_tutari, artir=True):
        ...
        try:
            ...
            return False, f"Kasa/banka bakiyesi güncellenemedi: {e}"
    ...
    def toplu_musteri_ekle_guncelle(self, musteriler_data):
        """
        Excel'den gelen müşteri listesini veritabanına ekler veya mevcutları günceller.
        """
        if not musteriler_data:
        ...
        errors = []
        current_time = self.get_current_datetime_str()
        user_id = self.app.current_user[0] if self.app and self.app.current_user else None

        try:
            self.conn.execute("BEGIN TRANSACTION")
            
            for i, row in enumerate(musteriler_data):
                try:
                    musteri_kodu = str(row[0]).strip()
                    ad = str(row[1]).strip()
                    ...
                    self.c.execute("SELECT id FROM musteriler WHERE musteri_kodu = ?", (musteri_kodu,))
                    existing_customer = self.c.fetchone()
                    ...
                    if existing_customer:
                        ...
                        self.c.execute("""
                            ...
                            son_guncelleme_tarihi_saat=?, son_guncelleyen_kullanici_id=? 
                            WHERE id=?
                        """, (ad, telefon, adres, vergi_dairesi, vergi_no, current_time, user_id, existing_customer[0]))
                    else:
                        # Yeni Ekleme
                        self.c.execute("""
                            INSERT INTO musteriler (musteri_kodu, ad, telefon, adres, vergi_dairesi, vergi_no, olusturma_tarihi_saat, olusturan_kullanici_id) 
                            VALUES (?,?,?,?,?,?,?,?)
                        """, (musteri_kodu, ad, telefon, adres, vergi_dairesi, vergi_no, current_time, user_id))
                    
                    success_count += 1
                except Exception as e:
                    errors.append(f"Satır {i+2} ({musteri_kodu}): {e}")
                    error_count += 1
    ...
    def toplu_tedarikci_ekle_guncelle(self, tedarikciler_data):
        """
        Excel'den gelen tedarikçi listesini veritabanına ekler veya mevcutları günceller.
        """
        ...
        try:
            ...
            for i, row in enumerate(tedarikciler_data):
                try:
                    ...
                    self.c.execute("SELECT id FROM tedarikciler WHERE tedarikci_kodu = ?", (tedarikci_kodu,))
                    existing_supplier = self.c.fetchone()
    ...
    def safe_float(self, val):
        ...
        """
        if val is None: # <<<< BU KONTROLÜ EKLEYİN >>>>
            return 0.0
        ...
        try:
            ...
            # Virgülü ondalık ayraç olarak koru, noktayı kaldır.
            # Örneğin: "1.234,56 TL" -> "1234,56"
            # "1,234.56 %" -> "1234.56" (bu durum için aşağıda ele alınacak)
            cleaned_val_str = s_val.replace(' ', '').replace('TL', '').replace('₺', '').replace('%', '')

            # Türkçe ve İngilizce ondalık/binlik ayrımı kontrolü:
            # Eğer stringde hem nokta hem virgül varsa, hangisinin ondalık olduğuna karar ver.
            # Genellikle sonuncusu ondalıktır. Örneğin: "1.234,56" -> virgül ondalık
